<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title><?php echo $page_title; ?> - Josi Movies</title>
    
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css" rel="stylesheet">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css">
    <link rel="stylesheet" href="assets/css/styles.css">
</head>
<body>
    <nav class="navbar navbar-expand-lg bg-danger navbar-dark">
        <div class="container">
            <a class="navbar-brand fw-bold" href="index.php">Josi Movies</a>
            
            <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarNav">
                <span class="navbar-toggler-icon"></span>
            </button>
            
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav me-auto">
                    <li class="nav-item">
                        <a class="nav-link" href="index.php">Home</a>
                    </li>
                    
                    <li class="nav-item dropdown">
                        <a class="nav-link dropdown-toggle" href="#" data-bs-toggle="dropdown">
                            Genres
                        </a>
                        <ul class="dropdown-menu">
                            <li><a class="dropdown-item" href="index.php?genre=Action">Action</a></li>
                            <li><a class="dropdown-item" href="index.php?genre=Drama">Drama</a></li>
                            <li><a class="dropdown-item" href="index.php?genre=Comedy">Comedy</a></li>
                            <li><a class="dropdown-item" href="index.php?genre=Horror">Horror</a></li>
                            <li><a class="dropdown-item" href="index.php?genre=Romance">Romance</a></li>
                            <li><a class="dropdown-item" href="index.php?genre=Sci-Fi">Sci-Fi</a></li>
                        </ul>
                    </li>
                    
                    <li class="nav-item">
                        <a class="nav-link" href="index.php?type=Series">Series</a>
                    </li>
                </ul>
                
                <span class="navbar-text text-white">
                    Addis Ababa, 22 Mazoria
                </span>
                
                <?php if (!is_admin_logged_in()): ?>
                    <a href="login.php" class="btn btn-outline-light btn-sm ms-3">Admin Login</a>
                <?php else: ?>
                    <a href="admin/dashboard.php" class="btn btn-outline-light btn-sm ms-3">Admin Panel</a>
                    <a href="logout.php" class="btn btn-light btn-sm ms-2">Logout</a>
                <?php endif; ?>
            </div>
        </div>
    </nav>
    
    <div class="container mt-4">